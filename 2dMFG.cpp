...
void intialize_rho_top(double* rho,double base,int n1,int n2,int nt,double dx,double dy,double dt){
	...
	for(int i=0;i<n2;++i){
		for(int j=0;j<n1;++j){
			...
			rho[i*n1+j] = gaussian(x,y,mux,muy,sigmax,sigmay)+0.05;
			sum += rho[i*n1+j];
		}
	}
...
void initialize_f_g(double* f,double* g,int n1,int n2,double dx,double dy){
	...
	for(int i=0;i<n2;++i){
		for(int j=0;j<n1;++j){
			...
			f[i*n1+j] = gaussian(x,y,0.5,0.5,0.05,0.05)+0.01;
			g[i*n1+j] = gaussian(x,y,0.5,0.2,0.05,0.05)+0.01;
...
public:
    ...
    void update_rho(double* rho,const double* f,const double* g){
    		...
    		for(int n=1;n<nt;++n){
    			for(int i=0;i<n2;++i){
    				for(int j=0;j<n1;++j){
    					...
    					if(j==0){
    						mxval=0.5*(mx[n*n1*n2+i*n1+j]);
    					}else{
    						mxval=0.5*(mx[n*n1*n2+i*n1+j]+mx[n*n1*n2+i*n1+j-1]);
    					}

    					if(i==0){
    						myval=0.5*(my[n*n1*n2+i*n1+j]);
    					}else{
    						myval=0.5*(my[n*n1*n2+i*n1+j]+my[n*n1*n2+(i-1)*n1+j]);
    					}
    					double mval=sqrt(mxval*mxval+myval*myval);
    					double rhoval=rho[n*n1*n2+i*n1+j];
    					double fval=f[i*n1+j];
    					double gval=g[i*n1+j];
    					double dtPhi=0;

    					if(n==nt-1){
    						dtPhi=(Phi[(n)*n1*n2+i*n1+j]-Phi[(n-1)*n1*n2+i*n1+j])/(1.0*dt);	
    					}else{
    						dtPhi=(Phi[(n+1)*n1*n2+i*n1+j]-Phi[(n-1)*n1*n2+i*n1+j])/(2.0*dt);
    					}

    					if(n==nt-1){
    						Fval  = -pow(mval,2)/(2*pow(rhoval,2)) + c1*log(rhoval) + fval + c1 + dtPhi + 1.0/tau*(rhoval-rhoTmp[n*n1*n2+i*n1+j]) + 1.0/dt * (c2*log(rhoval*exp(-gval/c2)) + c2 - Phi[(nt-1)*n1*n2+i*n1+j]);
    						DFval = pow(mval,2)/pow(rhoval,3) + c1/rhoval + 1.0/tau + 1.0/dt * (c2/rhoval);
    					}else{
    						Fval  = -pow(mval,2)/(2*pow(rhoval,2)) + c1*log(rhoval) + fval + c1 + dtPhi + 1.0/tau*(rhoval-rhoTmp[n*n1*n2+i*n1+j]);
    						DFval = pow(mval,2)/pow(rhoval,3) + c1/rhoval + 1.0/tau;
    					}

    					rho[n*n1*n2+i*n1+j] = fmax(0.00001,rhoval-0.01*Fval/DFval);

    					error += fabs(Fval/DFval);
    				}
    			}
    	...
    	// 		}
    		...
    		if(error*dx*dy*dt<tolerance_newton){
    			break;
    		}
    	}
    ...
    void update_phi(poisson_solver& fftps){
		for(int n=0;n<nt;++n){
			for(int i=0;i<n2;++i){
		    	for(int j=0;j<n1;++j){
			    	...
			    	if(n==0){
			    		dtrho=1.0/dt*(rhoTmp[(n+1)*n1*n2+i*n1+j]-rhoTmp[n*n1*n2+i*n1+j]);
			    	}else if(n==nt-1){
			    		dtrho=1.0/dt*(rhoTmp[(n)*n1*n2+i*n1+j]-rhoTmp[(n-1)*n1*n2+i*n1+j]);
			    	}else{
			    		dtrho=0.5/dt*(rhoTmp[(n+1)*n1*n2+i*n1+j]-rhoTmp[(n-1)*n1*n2+i*n1+j]);
			    	}
	 ...
	 //    }
		...
		// 				// double dtrho=0.5/dt*(rhoTmp[(n+1)*n1*n2+i*n1+j]-rhoTmp[n*n1*n2+i*n1+j]);
		// 				// double nablamx=calculate_grad_mx(mxTmp,n,i,j);
		// 				// double nablamy=calculate_grad_my(myTmp,n,i,j);
		// 				// fftps.workspace[i*n1+j]=-(dtrho+nablamx+nablamy);	
		// 			}else if(n==nt-1){
		// 				fftps.workspace[i*n1+j]=0;
		// 				// double dtrho=0.5/dt*(rhoTmp[n*n1*n2+i*n1+j]-rhoTmp[(n-1)*n1*n2+i*n1+j]);
		// 				// double nablamx=calculate_grad_mx(mxTmp,n,i,j);
		// 				// double nablamy=calculate_grad_my(myTmp,n,i,j);
		// 				// fftps.workspace[i*n1+j]=-(dtrho+nablamx+nablamy);
		// 			}else{
