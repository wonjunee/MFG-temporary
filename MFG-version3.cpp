...
void initialize_f_g(double* f,double* g,int n1,int n2,double dx,double dy){
    ...
    for(int i=0;i<n2;++i){
        for(int j=0;j<n1;++j){
            ...
            g[i*n1+j] = gaussian(x,y,0.1,0.1,0.05,0.05) + gaussian(x,y,0.9,0.1,0.05,0.05);
...
public:
    ...
    double calculate_energy(const double* rho,const double* f,const double* g){
        double sum1=0;
        double sum2=0;
        ...
        for(int n=1;n<nt;++n){
            for(int i=0;i<n2;++i){
                for(int j=0;j<n1;++j){
                    ...
                    if(rhoval>0){
                        // sum1+=pow(mval,2)/(2.0*rhoval) + c1*rhoval*log(rhoval) + rhoval*fval;    
                        sum1 += pow(mval,2)/(2.0*rhoval)  + rhoval*Psi[n*n1*n2+i*n1+j] - c1*exp( (Psi[n*n1*n2+i*n1+j]-fval)/c1-1 );
                    }
                }
            }
        }
        ...
        for(int i=0;i<n2;++i){
            for(int j=0;j<n1;++j){
                ...
                if(rhoval/g[i*n1+j]>0){
                    sum2+=c2*rhoval*log(rhoval/g[i*n1+j]);
                }
            }
        }
    ...
    double calculate_dual(const double* rho,const double* f,const double* g){
        ...
        for(int n=0;n<nt-1;++n){
            for(int k=0;k<n1*n2;++k){
                sum0 += c1 * exp( (Psi[n*n1*n2+k]-f[k])/c1 - 1);
            }
        }

        for(int i=0;i<n2;++i){
            for(int j=0;j<n1;++j){
                sum1+=Phi[i*n1+j]*rho[i*n1+j];
                sum2+=exp(Phi[(nt-1)*n1*n2+i*n1+j]/c2) * g[i*n1+j];
            }
        }

        return - sum0*dx*dy*dt + sum1*dx*dy - c2*log(sum2*dx*dy);
    }

    void run(poisson_solver& fftps, double* rho,const double* f,const double* g, int skip=1){
        ...
        for(iterPDHG=0; iterPDHG<max_iteration; ++iterPDHG){
            ...
            double energy=calculate_energy(rho,f,g);
            double dual=calculate_dual(rho,f,g);

            error=fabs((energy-previous_energy)/previous_energy);

            previous_energy=energy;

            double dual_gap=fabs(energy-dual);

            if((iterPDHG+1)%skip==0){
                cout<<"iter : "<<iterPDHG+1<< " energy : "<<energy<< " error : " << error <<" dual : " << dual <<" dual_gap : " << dual_gap<<endl;  
